...
**Your model, local or remote.** Point Vera at any OpenAI-compatible embedding or reranker endpoint, whether that's a remote API or a local server like llama.cpp. Everything else (indexing, storage, search logic) stays on your machine regardless. If you don't want to manage models at all, `vera setup` downloads two curated ONNX models that run locally, giving you the full three-stage pipeline without any network calls. Details in the [Model Backend](#model-backend) section.
